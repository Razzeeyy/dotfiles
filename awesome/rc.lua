...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey }, "space", function()
                local current_layout = awesome.xkb_get_layout_group()
                awesome.xkb_set_layout_group(current_layout + 1)
            end,
              {description = "cycle keyboard layout", group = "awesome"}),
...
awful.rules.rules = require("rules")(clientkeys,clientbuttons)
-- }}}
...
if has_autorun then
    local function run_once(cmd)
        if cmd then
            awful.spawn.with_shell("pgrep -f -u $USER '" .. cmd .. "' || (" .. cmd .. ")")
        end
    end
